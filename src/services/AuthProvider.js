...
function AuthProvider(props) {
  ...
  const login = async (username,password) => {
    ...
    const req = await dataService.Req(LoginEp,{email:username,password:password});
    switch(req.status) {
      case 200:
        const jwt = await req.json();
            dispatch({type:"login_success",payload:{email:username,token:jwt.token,token_type:jwt.token_type,expires_in:jwt.expires_in}});
            break;
        case 401:
